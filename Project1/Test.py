...
def repr_method(ClassShape):
    ...
    if classShape.StrList == "mt":
        return Pair(repr_header, Pair('        return "%s' % (ClassShape.name) + '()".format()', "mt"))

    return Pair(repr_header, Pair('        return "%s' % (ClassShape.name) + "({!r}%s" % (
    commasep(format_operator_list(num_fields(ClassShape.StrList) - 1))) + ')".format(%s%s)' % (
                                  "self." + ClassShape.StrList.first,
                                  commasep(fields_to_assignments(ClassShape.StrList.rest))), "mt"))
def num_fields(StrList):
	...
	if StrList == "mt":
		...
		return 1 + num_fields(StrList.rest)

# integer -> StrList
# Creates a StrList of !r format operators for how many field names there are for a specific class.
def format_operator_list(num):

	if num != 0:

		return Pair("{!r}", format_operator_list(num - 1))
...
class TestCases(unittest.TestCase):
	...
	def test_repr_method(self):
		...
		self.assertEqual(test, Pair('    def __repr__(self):', Pair('        return "Point({!r}, {!r}, {!r})".format(', Pair('        self.x, self.y, self.z)', 'mt'))))
	#18
