...
# lst, function, index -> list
# return lst sorted. function servers as less than conditional with addition of what index to search for music catalog list of lists.
...
def sort(lst, function):
	for i in range(length(lst)):
		current = lst.lst[i]
		j = i-1
		while (j>= 0) and function(current, lst.lst[j]):
			lst.lst[j+1] = lst.lst[j]
			j = j-1
		lst.lst[j+1] = current
	return lst