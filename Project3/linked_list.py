...
# lst, value, function -> lst
# inserts value into proper position of already sorted list
# def insert_sorted(lst,value,function):
#   pass
def insert_sorted(lst, value, function):
    if lst == None:
        return add(lst, 0, value)
        ...
        s = lst
        if function(value, s.first):
            return Pair(value, s)
        else:
            return Pair(s.first, insert_sorted(s.rest, value, function))
